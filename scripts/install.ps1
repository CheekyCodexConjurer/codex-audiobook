...
foreach ($profile in 'audiobook-structure.toml', 'audiobook-transcriber.toml', 'audiobook-verifier.toml') {
    Copy-Item -LiteralPath (Join-Path $agentsSource $profile) -Destination (Join-Path $agentsDest $profile) -Force
}
...
if ($RegisterMarketplace) {
    ...
    & $codex.Source plugin marketplace add $marketplaceSource
